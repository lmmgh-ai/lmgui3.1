...
function scene_gui:mousemoved(id, x, y, dx, dy, istouch, pre)
    ...
    if (select_view and pressed_views[1]) or input_state.select_point then -- and love.mouse.isDown(1) then --鼠标模式选中视图存在 执行选中视图滑动回调
        ...
        if i then
            ...
            --select_view.width = select_view.width + point[1] * dx
            --select_view.height = select_view.height + point[2] * dy
...
function scene_gui:mousepressed(id, x, y, dx, dy, istouch, pre) --pre短时间按下次数 模拟双击
    ...
    if id == 1 then
        ...
        --赋值选中视图
        input_state.select_view = scan(tree_views_children, nil, x, y)

        --赋值触控id视图
        pressed_views[id] = input_state.select_view
        --print(input_state.select_view)
        if input_state.select_view then
            return input_state.select_view
        end
        ]]
        --print(input_state.select_view, "选中赋值为空")
        --选中视图赋值为空
...
function scene_gui:mousereleased(id, x, y, dx, dy, istouch, pre) --pre短时间按下次数 模拟双击
    ...
    local select_view = input_state.select_view                  --选中视图
    --拖动改变父视图
    if select_view and point_view then
    ...
    elseif select_view and not point_view then
        if select_view:get_parent() then

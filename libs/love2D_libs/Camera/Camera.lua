...
function Camera:new(x, y, width, height)
    ...
    -- 计算视口中心(用于中心坐标系)
    cam.viewport.centerX = x - width / 2
    cam.viewport.centerY = y - height / 2
...
function Camera:attach()
    ...
    -- 变换到视口中心
    love.graphics.translate(self.viewport.centerX, self.viewport.centerY)
...
function Camera:world_to_scene_position(worldX, worldY)
    -- 转换为相对于视口中心的坐标
    local x = worldX - self.viewport.centerX
    local y = worldY - self.viewport.centerY
...
function Camera:scene_to_world_position(sceneX, sceneY)
    ...
    -- 转换为屏幕坐标
    return rx + self.viewport.centerX, ry + self.viewport.centerY
end

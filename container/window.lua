...
function window:_event_intercept(x, y, child)
    --判断按钮 高优先级
    -- 检查是否点击了调整大小区域
    if self:isMouseInResizeArea() and not self.isMaximized and not self.isMinimized then
...
function window:mousemoved(id, x, y, dx, dy, istouch, pre)
    ...
    if self.isResizing and not self.isMaximized and not self.isMinimized then
        local value = (#self.buttons + 1) * self.titleBarHeight
        self.width = math.max(self.min_width, self.width + dx)
        self.height = math.max(self.min_height, self.height + dy)
        -- 更新按钮位置
        self:updateButtons()
        return --拦截拖动
    end

...
function border_container:get_local_Position(x, y, child)
    ...
    if child then
        ...
        if label then
            -- print(child.type)
            if label == "main" then
                -- print(left.x + left.width, top.y + top.height, right.x - self.x, bottom.y - self.y)
                --print(x, y)
                --print(self.point_in_rect(x, y, left.x + left.width, top.y + top.height, right.x - self.x, bottom.y - self.y))

...
--不受滑动影响 仅自身判断使用
function slider_container:get_local_noSlider_Position(x, y, child)
    local parent = self.parent
    local x1 = x - self.x
    local y1 = y - self.y
    if parent then
...
function slider_container:_event_intercept(x, y, child)
    --如果在视图内
    --转换局部坐标
    local x1, y1 = self:get_local_noSlider_Position(x, y, self)
    --点击在视图内
...
function slider_container:mousemoved(id, x, y, dx, dy, istouch, pre) --滑动回调
    local x1, y1 = self:get_local_noSlider_Position(x, y)
